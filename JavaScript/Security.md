...
No origin can access another origin data.
However, cookies have a different notion of origin.

## CORS

You can make a request to a resource,
however, if remote origin does not match your current origin,
then the browser won't allow you to access the response in your client code.

Servers control CORS by settings HTTP headers.

## Cookies

The HTTP Protocol is stateless. One uses Cookies to introduce state.
Cookies have a different notion of origin: Site != origin.

Cookies might be hijacked and so sessions might be stolen.
Use two-factor authentication to prevent this.
